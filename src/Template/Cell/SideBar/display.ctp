      ...
      <?= $this->Html->link('<i class="fa fa-file" aria-hidden="true"></i> &nbsp; PROTOCOLS', ['controller' => 'Applications', 'action' => 'index', 'prefix' => $prefix, 'plugin' => false ], array('escape' => false)); ?>      
        ...
        <?php if (($prefix == 'manager' || strpos($prefix, 'evaluator') !== false) && $this->request->params['controller'] == 'Applications' ) { ?>
          <ul class="nav van-<?= $prefix ?>">
            ...
            <li class="<?= (isset($this->request->query['status']) && $this->request->query['status'] == 'DirectorAuthorize' or (isset($this->request->query['status'])) && $this->request->query['status'] == 'Authorize') ? 'active' : ''; ?>"><?= $this->Html->link('<b>10.</b> Authorized'. $AllAuthorize , ['controller' => 'Applications', 'action' => 'index', 'status' => 'DirectorAuthorize', 'prefix' => $prefix, 'plugin' => false ], array('escape' => false)); ?> </li>
            <li class="<?= (isset($this->request->query['status']) && $this->request->query['status'] == 'DirectorAuthorize') ? 'active' : ''; ?>"><?= $this->Html->link('<b style="padding-left: 25px;"></b> Director'. $DirectorAuthorize , ['controller' => 'Applications', 'action' => 'index', 'status' => 'DirectorAuthorize', 'prefix' => $prefix, 'plugin' => false ], array('escape' => false)); ?> </li>
