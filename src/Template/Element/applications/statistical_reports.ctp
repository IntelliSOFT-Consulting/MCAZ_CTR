...
<div class="row">
    <div class="col-xs-12">
        ...
        foreach ($statisticals as $statistical) {
            if ($statistical->evaluation_type == 'Initial') {

                // Added section
                $statistical_edit = [];
                $resa = [];
                $res = [];
                foreach ($statistical->statistical_edits as $key => $value) {
                    $res[$key] = $value->toArray();
                }
                $resa = Hash::flatten($res);
                foreach ($resa as $key => $value) {
                    if (!isset($statistical_edit[substr($key, strrpos($key, '.') + 1)])) {
                        $statistical_edit[substr($key, strrpos($key, '.') + 1)][] =
                            ($statistical[substr($key, strrpos($key, '.') + 1)] != $value) ? $value : null;
                    } else {
                        $statistical_edit[substr($key, strrpos($key, '.') + 1)][] =
                            (end($statistical_edit[substr($key, strrpos($key, '.') + 1)]) != $value) ? $value : null;
                    }
                } ?>

                <?php
                $evec = count($statistical->statistical_edits);
                $eved = $statistical->statistical_edits;
                $hlis = [];
                ?>

                <div class="thumbnail amend-form">
                    <a class="btn btn-primary" role="button" data-toggle="collapse" href="#<?= $statistical->created->i18nFormat('dd-MM-yyyy_HH_mm_ss') ?>" aria-expanded="false" aria-controls="<?= $statistical->created->i18nFormat('dd-MM-yyyy_HH_mm_ss') ?>">
                    ...
                    if (
                        ...
                        and count(array_filter(Hash::extract($statisticals, '{n}.chosen'), 'is_numeric')) < 1
                    ) {
                        echo $this->Form->postLink(
                            '<span class="label label-info">Edit</span>',
                            ['action' => 'view', $application->id, '?' => ['stat_id' => $statistical->id]],
                            ['data' => ['stat_id' => $statistical->id], 'escape' => false, 'confirm' => __('Are you sure you want to edit statistical {0}?', $statistical->id)]
                        );
                        echo "&nbsp;";
                        if ($prefix == 'evaluator') echo $this->Form->postLink(
                            '<span class="label label-success">Copy & Finalize <small>(initial review)</small></span>',
                            ['action' => 'view', $application->id, '?' => ['stat_fn' => $statistical->id]],
                            ['data' => ['stat_fn' => $statistical->id], 'escape' => false, 'confirm' => __('Are you sure you want to copy and finalize statistical {0}?', $statistical->id)]
                        );
                        echo "&nbsp;";
                        for ($i = 0; $i < count($statistical->statistical_edits); $i++) {
                            if ($this->request->params['_ext'] != 'pdf' and $statistical->statistical_edits[$i]['user']['group_id'] == 2) {
                                if ($prefix == 'evaluator') echo $this->Form->postLink(
                                    '<span class="label label-default">Copy & Finalize <small>(manager version # ' . ($i + 1) . ')</small></span>',
                                    ['action' => 'view', $application->id, '?' => ['stat_fn' => $statistical->statistical_edits[$i]['id']]],
                                    ['data' => ['stat_fn' => $statistical->statistical_edits[$i]['id']], 'escape' => false, 'confirm' => __('Are you sure you want to copy and finalize manager edit {0}?', ($i + 1))]
                                );
                                echo '&nbsp;';
                            }
                        }
                    ...
                    if (
                        ...
                        echo $this->Form->postLink(
                            ...
                            [],
                            ['data' => ['statistical_id' => $statistical->id], 'escape' => false, 'confirm' => __('Are you sure you want to create new tracked change for statistical {0}?', $statistical->id)]
                        );
                        echo '&nbsp;';
                        for ($i = 0; $i < count($statistical->statistical_edits); $i++) {
                            if ($this->request->params['_ext'] != 'pdf' and $statistical->statistical_edits[$i]['user_id'] == $this->request->session()->read('Auth.User.id')) {
                                echo $this->Form->postLink(
                                    '<span class="label label-success">Edit change # ' . ($i + 1) . '</span>',
                                    ['action' => 'view', $application->id, '?' => ['stat_id' => $statistical->statistical_edits[$i]['id']]],
                                    ['data' => ['stat_id' => $statistical->statistical_edits[$i]['id']], 'escape' => false, 'confirm' => __('Are you sure you want to edit tracked change {0}?', ($i + 1))]
                                );
                                echo '&nbsp;';
                            }
                        }
                ...
                <div class="<?= ($this->request->params['_ext'] != 'pdf') ? 'collapse' : ''; ?>" id="<?= $statistical->created->i18nFormat('dd-MM-yyyy_HH_mm_ss') ?>">
