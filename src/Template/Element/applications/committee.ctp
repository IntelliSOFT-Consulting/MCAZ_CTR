...
<?php if (
  !in_array("9", Hash::extract($application->application_stages, '{n}.stage_id')) 
  and  !in_array($application->approved, ['Authorize', 'DirectorAuthorize', 'Declined', 'DirectorGeneral']  )
) { ?>
  ...
  <?php if (!empty($application->evaluations)) { ?>
    ...
    <div class="row">
      <div class="col-xs-12">
        ...
        echo $this->Form->control('committee_reviews.100.decision', [
          'type' => 'radio',
...
<script type="text/javascript">
  if ($('#committee-reviews-100-internal-review-comment').length) {
    CKEDITOR.replace('committee-reviews-100-internal-review-comment');
  }
  if ($('#committee-reviews-100-applicant-review-comment').length) {
