...
<div class="row">
    <div class="col-xs-12">
        ...
        foreach ($non_clinicals as $nonclinical) {
            if ($nonclinical->evaluation_type == 'Initial') {

                // Added section
                $nonclinical_edit = [];
                $resa = [];
                $res = [];
                foreach ($nonclinical->non_clinical_edits as $key => $value) {
                    $res[$key] = $value->toArray();
                }
                $resa = Hash::flatten($res);
                foreach ($resa as $key => $value) {
                    if (!isset($nonclinical_edit[substr($key, strrpos($key, '.') + 1)])) {
                        $nonclinical_edit[substr($key, strrpos($key, '.') + 1)][] =
                            ($nonclinical[substr($key, strrpos($key, '.') + 1)] != $value) ? $value : null;
                    } else {
                        $nonclinical_edit[substr($key, strrpos($key, '.') + 1)][] =
                            (end($nonclinical_edit[substr($key, strrpos($key, '.') + 1)]) != $value) ? $value : null;
                    }
                } ?>

                <?php
                $evec = count($nonclinical->non_clinical_edits);
                $eved = $nonclinical->non_clinical_edits;
                $hlis = [];
                ?>

                <div class="thumbnail amend-form">
                    <a class="btn btn-primary" role="button" data-toggle="collapse" href="#<?= $nonclinical->created->i18nFormat('dd-MM-yyyy_HH_mm_ss') ?>" aria-expanded="false" aria-controls="<?= $nonclinical->created->i18nFormat('dd-MM-yyyy_HH_mm_ss') ?>">
                    ...
                    // Copy Edit
                    if (
                        $this->request->params['_ext'] != 'pdf' and ($nonclinical->user_id == $this->request->session()->read('Auth.User.id'))
                        and count(array_filter(Hash::extract($non_clinicals, '{n}.chosen'), 'is_numeric')) < 1
                    ) {
                        echo $this->Form->postLink(
                            '<span class="label label-info">Edit</span>',
                            ['action' => 'view', $application->id, '?' => ['non_cnl_id' => $nonclinical->id]],
                            ['data' => ['non_cnl_id' => $nonclinical->id], 'escape' => false, 'confirm' => __('Are you sure you want to edit nonclinical {0}?', $nonclinical->id)]
                        );
                        echo "&nbsp;";
                        if ($prefix == 'evaluator') echo $this->Form->postLink(
                            '<span class="label label-success">Copy & Finalize <small>(initial review)</small></span>',
                            ['action' => 'view', $application->id, '?' => ['non_cp_fn_cnl' => $nonclinical->id]],
                            ['data' => ['non_cp_fn_cnl' => $nonclinical->id], 'escape' => false, 'confirm' => __('Are you sure you want to copy and finalize nonclinical {0}?', $nonclinical->id)]
                        );
                        echo "&nbsp;";
                        for ($i = 0; $i < count($nonclinical->non_clinical_edits); $i++) {
                            if ($this->request->params['_ext'] != 'pdf' and $nonclinical->non_clinical_edits[$i]['user']['group_id'] == 2) {
                                if ($prefix == 'evaluator') echo $this->Form->postLink(
                                    '<span class="label label-default">Copy & Finalize <small>(manager version # ' . ($i + 1) . ')</small></span>',
                                    ['action' => 'view', $application->id, '?' => ['non_cp_fn_cnl' => $nonclinical->non_clinical_edits[$i]['id']]],
                                    ['data' => ['non_cp_fn_cnl' => $nonclinical->non_clinical_edits[$i]['id']], 'escape' => false, 'confirm' => __('Are you sure you want to copy and finalize manager edit {0}?', ($i + 1))]
                                );
                                echo '&nbsp;';
                            }
                        }
                    ...
                    if (
                        ...
                        echo $this->Form->postLink(
                            ...
                            [],
                            ['data' => ['non_clinical_id' => $nonclinical->id], 'escape' => false, 'confirm' => __('Are you sure you want to create new tracked change for nonclinical {0}?', $nonclinical->id)]
                        );
                        echo '&nbsp;';
                        for ($i = 0; $i < count($nonclinical->non_clinical_edits); $i++) {
                            if ($this->request->params['_ext'] != 'pdf' and $nonclinical->non_clinical_edits[$i]['user_id'] == $this->request->session()->read('Auth.User.id')) {
                                echo $this->Form->postLink(
                                    '<span class="label label-success">Edit change # ' . ($i + 1) . '</span>',
                                    ['action' => 'view', $application->id, '?' => ['non_cnl_id' => $nonclinical->non_clinical_edits[$i]['id']]],
                                    ['data' => ['non_cnl_id' => $nonclinical->non_clinical_edits[$i]['id']], 'escape' => false, 'confirm' => __('Are you sure you want to edit tracked change {0}?', ($i + 1))]
                                );
                                echo '&nbsp;';
                            }
                        }
                    ...
                    if (
                        ...
                        and is_null($nonclinical->chosen)
                    ) {
                        echo $this->Form->postLink(
                            '<span class="label label-success active">Approve the Review?</span>',
                            ['action' => 'attachNonClinicalSignature', $nonclinical->id, 'prefix' => $prefix],
                            ['escape' => false, 'confirm' => 'Are you sure you want to attach your signature to Review?', 'class' => 'label-link']
                        );
                    }
                    if (
                        $this->request->params['_ext'] != 'pdf' and !empty($nonclinical->chosen)
                        and in_array($nonclinical->chosen, Hash::extract($non_clinicals, '{n}.chosen'))
                    ) {
                ...
                <div class="<?= ($this->request->params['_ext'] != 'pdf') ? 'collapse' : ''; ?>" id="<?= $nonclinical->created->i18nFormat('dd-MM-yyyy_HH_mm_ss') ?>">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-xs-12">
                                            <h4 class="text-center">
                                                <?php
                                                echo ($nonclinical->user->dir) ? "<span>" . $nonclinical->user->name . ": </span><img src='" . $this->Url->build(substr($nonclinical->user->dir, 8) . '/' . $nonclinical->user->file, true) . "' style='width: 30%;' alt=''>" : '';
                                                ?></h4>
                                            <?= ($nonclinical->chosen) ? $this->cell('Signature', [$nonclinical->chosen]) : ''; ?>
                                        </div>
                                    </div>
                                    <br>
                                </td>
