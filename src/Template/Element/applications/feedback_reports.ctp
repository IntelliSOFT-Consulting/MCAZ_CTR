    ...
    ?>
        <div class="thumbnail amend-form">
            <a class="btn btn-<?php
                          ...
                          ?>" role="button" data-toggle="collapse" href="#<?= $cn ?>" aria-expanded="false"
                aria-controls="<?= $cn ?>">
        ...
        ?>
            <a href="#">
                <span class="label label-info" data-toggle="modal" data-target="#evalAssignModal<?= $cn ?>"> Assign
                    Evalator </span>
            </a>

            <!-- Modal -->
            <div class="modal fade" id="evalAssignModal<?= $cn ?>" tabindex="-1" role="dialog"
                aria-labelledby="myModalLabel<?= $cn ?>">
                ...
                echo $this->Form->create(['schema' => ['assigned_to'], 'defaults' => ['assigned_to' => $xf['assigned_to']]], ['type' => 'file', 'url' => [
                  'controller' => 'Comments', 'action' => 'submitAll', $cn, 'prefix' => $prefix,
                  '?' => ['cf_ae' => $cn]
                ]]);
                ?>
        ...
        if ($prefix == 'manager' and $bo > 0 and $bo != $co) {
          echo $this->Form->postLink(
            ...
            [
              'data' => [
                'id' => $cn, 'feedbacks' => Hash::extract(Hash::extract($comments, "{n}[submitted=2]"), "{n}[model_id=$cn].id"), 'submitted' => 2, 'foreign_key' => $application->id,
        ...
        ?>
            <div class="<?= ($this->request->params['_ext'] == 'pdf' || !empty($this->request->query('cf_id'))) ? '' : 'collapse'; ?>"
                id="<?= $cn ?>">
              ...
              ?>
                        ...
                        <tr class="<?php
                            ...
                            <td>
                                ...
                                <?php
                        if (
                          $this->request->params['_ext'] != 'pdf' and ($comment->user_id == $this->request->session()->read('Auth.User.id'))
                        ...
                        ?>
                                ...
                                <a href="#">
                                    <span class="label label-warning" data-toggle="modal"
                                        data-target="#revertModal<?= $comment->id ?>"> Revert </span>
                                </a>

                                <!-- Modal -->
                                <div class="modal fade" id="revertModal<?= $comment->id ?>" tabindex="-1" role="dialog"
                                    aria-labelledby="myModalLabel<?= $comment->id ?>">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                            ...
                                            <?php
                                echo $this->Form->create($comment, ['type' => 'file', 'url' => [
                                  'controller' => 'Comments', 'action' => 'submit', $comment->id, 'prefix' => $prefix,
                                  '?' => ['cf_rv' => $comment->id]
                                ]]);
                                ?>
                                            <div class="modal-body">
                        ...
                        <tr class="<?php
                            ...
                            <!-- <td></td>       -->
                            <td colspan="2" class="evaluation-commentsa" data-type="wysihtml5"
                                data-pk="<?= $comment->id ?>"
                                data-url="<?= $this->Url->build(['controller' => 'Comments', 'action' => 'submit', $comment->id,  'prefix' => $prefix]); ?>"
                                data-name="review" data-title="Enter evaluator's comment">
                                <?php
                        $odipo = $bodipo = false;
                        ...
                        ?>
                                ...
                                <!-- <a href="#"> -->
                                <button class="btn btn-warning btn-xs" id="#evalfeedModal<?= $comment->id ?>"
                                    onclick='$("#evalfeedModal<?= $comment->id ?>").toggle()'> Evaluator's feedback
                                </button>
                                <!-- </a> -->
                                <?php } ?>

                                <!-- Modal -->
                                <div id="evalfeedModal<?= $comment->id ?>"
                                    aria-labelledby="myModalLabel<?= $comment->id ?>" style="display: none;">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                              ...
                              ?>
                                ...
                                ?>
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button type="submit" class="btn btn-primary btn-sm" name="ef_submitted"
                                                    value="<?= ($comment->ef_submitted == 1 or empty($comment->ef_submitted)) ? 1 : $comment->ef_submitted ?>"><i
                        ...
                        ?>
                                ...
                                <a href="#">
                                    <span class="label label-warning" data-toggle="modal"
                                        data-target="#revertefModal<?= $comment->id ?>"> Revert </span>
                                </a>

                                <!-- Modal -->
                                <div class="modal fade" id="revertefModal<?= $comment->id ?>" tabindex="-1"
                                    role="dialog" aria-labelledby="revertefModalLabel<?= $comment->id ?>">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <button type="button" class="close" data-dismiss="modal"
              ...
              endforeach; ?>
                  ...
                  ?>
                                <button type="submit" class="btn btn-success btn-sm" name="submitted" value="2"
                                    onclick="return confirm('Are you sure you wish to submit the final recommendation?');">
                    ...
                    ?>
                                ...
                                <a href="#">
                                    <span class="label label-warning" data-toggle="modal"
                                        data-target="#requestModal<?= $recommandation->id ?>"> Request changes </span>
                                </a>

                                <!-- Modal -->
                                <div class="modal fade" id="requestModal<?= $recommandation->id ?>" tabindex="-1"
                                    role="dialog" aria-labelledby="myModalLabel<?= $recommandation->id ?>">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <button type="button" class="close" data-dismiss="modal"
                                            ...
                                            <?php
                          echo $this->Form->create($recommandation, ['type' => 'file', 'url' => [
                            'controller' => 'Comments', 'action' => 'submit', $recommandation->id, 'prefix' => $prefix,
