...
class AmendmentsBaseController extends AppController
{
    ...
    public function index()
    {
        ...
        if ($this->request->params['_ext'] === 'csv') {
            ...
            $_extract = ['protocol_no', 'public_title', 'scientific_title', 'public_contact_name', 'public_contact_designation', 'public_contact_email', 'public_contact_phone', 'public_contact_postal', 'public_contact_affiliation', 'scientific_contact_name', 'scientific_contact_designation', 'scientific_contact_email', 'scientific_contact_phone', 'scientific_contact_postal', 'scientific_contact_affiliation', 'countries_recruitment', 'abstract_of_study', 'version_no', 'date_of_protocol', 'study_drug', 'product_type_chemical', 'product_type_chemical_name', 'product_type_biologicals', 'product_type_biologicals_name', 'product_type_medical_device', 'product_type_medical_device_name', 'protocol_version', 
            function ($row) { return implode('|', Hash::extract($row['investigator_contacts'], '{n}.given_name')); }, 
            function ($row) { return implode('|', Hash::extract($row['investigator_contacts'], '{n}.date_of_birth')); }, 
            function ($row) { return implode('|', Hash::extract($row['investigator_contacts'], '{n}.qualification')); }, 
            function ($row) { return implode('|', Hash::extract($row['investigator_contacts'], '{n}.professional_address')); }, 
            function ($row) { return implode('|', Hash::extract($row['investigator_contacts'], '{n}.telephone')); }, 
            function ($row) { return implode('|', Hash::extract($row['investigator_contacts'], '{n}.email')); }, 
            ...
            function ($row) { return implode('|', Hash::extract($row['sponsors'], '{n}.sponsor')); }, 
            function ($row) { return implode('|', Hash::extract($row['sponsors'], '{n}.contact_person')); }, 
            function ($row) { return implode('|', Hash::extract($row['sponsors'], '{n}.address')); }, 
            function ($row) { return implode('|', Hash::extract($row['sponsors'], '{n}.telephone_number')); }, 
            function ($row) { return implode('|', Hash::extract($row['sponsors'], '{n}.fax_number')); }, 
            function ($row) { return implode('|', Hash::extract($row['sponsors'], '{n}.cell_number')); }, 
            function ($row) { return implode('|', Hash::extract($row['sponsors'], '{n}.email_address')); }, 
            ...
            function ($row) { return implode('|', Hash::extract($row['participants'], '{n}.name')); }, 
            function ($row) { return implode('|', Hash::extract($row['participants'], '{n}.occupation')); }, 
            function ($row) { return implode('|', Hash::extract($row['participants'], '{n}.address')); }, 
            function ($row) { return implode('|', Hash::extract($row['participants'], '{n}.date_of_birth')); }, 
            ...
            function ($row) { return implode('|', Hash::extract($row['site_details'], '{n}.contact_person')); }, 
            function ($row) use($_provinces) { return implode('|', Hash::map($row['site_details'], '{n}.province_id', 
                function($val) use($_provinces) { return $_provinces[$val];}
        )); }, 
            'multiple_countries', 'multiple_member_states', 'multi_country_list', 'drug_name', 'quantity_excemption',             
            function ($row) { return implode('|', Hash::extract($row['medicines'], '{n}.medicine_name')); }, 
            function ($row) { return implode('|', Hash::extract($row['medicines'], '{n}.quantity_required')); }, 
            ...
            function ($row) { return implode('|', Hash::extract($row['committees'], '{n}.name')); }, 
            function ($row) { return implode('|', Hash::extract($row['committees'], '{n}.address')); },             
            function ($row) { return implode('|', Hash::extract($row['committees'], '{n}.telephone_number')); }, 
            function ($row) { return implode('|', Hash::extract($row['committees'], '{n}.email_address')); }, 
            ...
            function ($row) { return implode('|', Hash::extract($row['mc10_forms'], '{n}.file')); }, 
            function ($row) { return implode('|', Hash::extract($row['receipts'], '{n}.file')); },
            function ($row) { return implode('|', Hash::extract($row['receipts'], '{n}.description')); },
            function ($row) { return implode('|', Hash::extract($row['application_stages'], '{n}.stage.name')); }, 
            function ($row) { return implode('|', Hash::extract($row['application_stages'], '{n}.stage_date')); },
            function ($row) { return implode('|', Hash::extract($row['application_stages'], '{n}.created')); },
            //not visible on form
            function ($row) use($_users) { return implode('|', Hash::map($row['assign_evaluators'], '{n}.user_id', 
                function($val) use($_users) { return $_users[$val];}
        )); },  
            function ($row) use($_users) { return implode('|', Hash::map($row['assign_evaluators'], '{n}.assigned_to', 
                function($val) use($_users) { return $_users[$val];}
        )); },             
